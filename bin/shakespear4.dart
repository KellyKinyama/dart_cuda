...
class CharTokenizer {
  ...
  CharTokenizer(String text) {
    // Get unique characters and sort them for consistency
    chars = split(text.split(' ')).toList()..sort();
    // ; //..sort();
    // chars = chars.toSet().toList()..sort();
    stoi = {for (var i = 0; i < chars.length; i++) chars[i]: i};
    itos = stoi.map((k, v) => MapEntry(v, k));
  }
  ...
  Set<String> split(List<String> input) {
    final out = input.expand((toElements) {
      for (int i = 0; i < toElements.length; i++) {
        if (!characters.contains(toElements[i])) {
          return toElements.split(toElements[i]);
        }
      }
      return toElements.split(' ').toSet();

      // return toElements.split(pattern);
    });
    if (out.length == input.length) {
      return out.toSet();
    } else {
      return split(out.toList());
    }
  }

  List<int> encode(String s) {
    return split(s.split(' ')).map((c) => stoi[c] ?? 0).toList();
  }

  String decode(List<int> l) => l.map((i) => itos[i] ?? 'null').join(' ');
}
...
void main() async {
  ...
  final tokenizer = CharTokenizer(rawText.substring(0, 1000));
  ...
  print("tokenizer.vocabSize ${tokenizer.vocabSize}");
  print("vocab ${tokenizer.stoi}");
