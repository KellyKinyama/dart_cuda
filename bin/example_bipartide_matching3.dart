...
void main() {
  ...
  for (int epoch = 0; epoch <= 250; epoch++) {
    ...
    // Apply sigmoid to boxes if the model doesn't do it internally
    final logits = preds['logits']!.reshape([numQueries, numClasses + 1]);
    final boxes = preds['boxes']!.reshape([numQueries, 4]).sigmoid();
    ...
    final List<double> boxMaskRaw = List.filled(numQueries, 0.0);

    for (int q = 0; q < numQueries; q++) {
      int gtIdx = assignments[q];
      if (gtIdx != -1 && gtIdx < gtObjects.length) {
        alignedClassIds[q] = gtObjects[gtIdx]['class_id'];
        boxMaskRaw[q] = 1.0; // Only calculate box loss for matched queries
    ...
    final boxMask = Tensor.fromList([numQueries, 1], boxMaskRaw);
    tracker.addAll([alignedGtBoxes, boxMask]);

    // 3. Masked & Weighted Loss Calculation
    const double boxWeight = 10.0;
    const double classWeight = 1.0;

    final classLoss = logits.crossEntropy(alignedClassIds);
    final rawBoxDiff = (boxes - alignedGtBoxes).abs();
    final maskedBoxDiff = rawBoxDiff * boxMask; // Zeros out background queries

    final totalLoss =
        (classLoss * classWeight) + (maskedBoxDiff.sum() * boxWeight);
    tracker.addAll([classLoss, rawBoxDiff, maskedBoxDiff, totalLoss]);

    // 4. Backprop & Step
    totalLoss.backward();
    optimizer.step();

    if (epoch % 50 == 0 || epoch == 250) {
      print(
