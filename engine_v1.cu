...
__global__ void add_forward_kernel(float *a, float *b, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = a[i] + b[i];
}
__global__ void add_backward_kernel(float *grad_out, float *grad_a, float *grad_b, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
    {
...
__global__ void sub_forward_kernel(float *a, float *b, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = a[i] - b[i];
}
__global__ void sub_backward_kernel(float *grad_out, float *grad_a, float *grad_b, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
    {
...
__global__ void mul_forward_kernel(float *a, float *b, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = a[i] * b[i];
}
__global__ void mul_backward_kernel(float *data_a, float *data_b, float *grad_out, float *grad_a, float *grad_b, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
    {
        atomicAdd(&grad_a[i], data_b[i] * grad_out[i]);
        atomicAdd(&grad_b[i], data_a[i] * grad_out[i]);
    }
}
__global__ void div_forward_kernel(float *a, float *b, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = a[i] / b[i];
}
__global__ void div_backward_kernel(float *data_a, float *data_b, float *grad_out, float *grad_a, float *grad_b, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
    {
        float b_val = data_b[i];
        atomicAdd(&grad_a[i], (1.0f / b_val) * grad_out[i]);
        atomicAdd(&grad_b[i], (-data_a[i] / (b_val * b_val)) * grad_out[i]);
    }
}
__global__ void neg_forward_kernel(float *a, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = -a[i];
}
__global__ void neg_backward_kernel(float *grad_out, float *grad_a, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        atomicAdd(&grad_a[i], -grad_out[i]);
}
__global__ void pow_forward_kernel(float *a, float exponent, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = powf(a[i], exponent);
}
__global__ void pow_backward_kernel(float *data_a, float exponent, float *grad_out, float *grad_a, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        atomicAdd(&grad_a[i], (exponent * powf(data_a[i], exponent - 1.0f)) * grad_out[i]);
}
__global__ void relu_forward_kernel(float *a, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = a[i] > 0.0f ? a[i] : 0.0f;
}
__global__ void relu_backward_kernel(float *data_a, float *grad_out, float *grad_a, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        if (data_a[i] > 0.0f)
...
__global__ void tanh_forward_kernel(float *a, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = tanhf(a[i]);
}
__global__ void tanh_backward_kernel(float *data_out, float *grad_out, float *grad_a, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
    {
        float out_val = data_out[i];
        atomicAdd(&grad_a[i], (1.0f - out_val * out_val) * grad_out[i]);
    }
}
__global__ void exp_forward_kernel(float *a, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = expf(a[i]);
}
__global__ void exp_backward_kernel(float *data_out, float *grad_out, float *grad_a, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        atomicAdd(&grad_a[i], data_out[i] * grad_out[i]);
}
__global__ void log_forward_kernel(float *a, float *out, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        out[i] = logf(a[i]);
}
__global__ void log_backward_kernel(float *data_a, float *grad_out, float *grad_a, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n)
        atomicAdd(&grad_a[i], (1.0f / data_a[i]) * grad_out[i]);
}
...
extern "C" DLLEXPORT TensorHandle create_tensor(int rows, int cols, float *initial_data)
{
    ...
    if (initial_data != nullptr)
        cudaMemcpy(t->data_gpu, initial_data, byte_size, cudaMemcpyHostToDevice);
