...
class TransformerDecoder extends Module {
  ...
  /// MuZero Representation: Converts tokens into a latent state 's'
  Tensor getLatentState(
    List<int> idx,
    ...
    // 4. Extract only the last token's hidden state [1, embedSize]
    // In MuZero, the 'state' is the representation of the sequence so far.
    if (T == 1) return xNorm; // Already [1, embedSize]

    final lastRow = xNorm.slice(T - 1, 1);
    tracker.add(lastRow);
    return lastRow;
  }
