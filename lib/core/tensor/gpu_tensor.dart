// GPU Tensor class with autograd, FFI-backed by libmat_mul.so.
//
// CUDA bindings live in `cuda_engine.dart`; we re-export the `engine`
// singleton and `CudaEngine` class so existing
// `import 'package:dart_cuda/core/tensor/gpu_tensor.dart'` users continue
// to work unchanged.
...
// The C++ autograd graph stores raw `Tensor*` pointers in `_children` and
// captures raw pointers inside `_backward` closures. Dart's GC has no
// visibility into those references, so a `Finalizer` cannot safely free
// owning tensors automatically — a Dart wrapper can become unreachable
// while the underlying C++ struct is still required by some other
// tensor's backward pass. We therefore rely exclusively on explicit
// `dispose()` calls (see the `tracker` + `_safeCleanup` pattern in the
// examples). `dispose()` is a no-op on views and idempotent.
